...
bool Board::isValidLocation(char row, int column, char rowSource, int columnSource, Player player)
{
    ...
    if (this->state == mill)
    {
        ...
        if (positions[x][y].getStatus() == (3 - player.getPlayerID()))
        {
            yaPassed = true;
            /*if it is part of a mill...*/
            if (positions[x][y].checkIfPartOfMill(this->positions))
            {
                /*if the enemy player has no free pieces, or has only 3 pieces...*/
                if (BoardPlayers.at(player.getEnemy() - 1).getFreePieces() == 0 ||
...
void Board::doMovePhase1(char row, int column, Player &player)
{
    if (isValidLocation(row, column, 0, 0, player))
    {
        ...
        getCurrentPlayer().increaseFreePieces();
        switch (a.first)
        {
        case true:
...
bool Board::removePiece(char row, int column, Player player)
{
    ...
    if (isValidLocation(row, column, 0, 0, player))
    {
        ...
        BoardPlayers.at(player.getEnemy() - 1).decreaseFreePieces();
        ...
        //upon successful removal of a piece, update their free pieces since mill has been made.
        getCurrentPlayer().updateFreePieces();
        return toReturn;
    }
    return toReturn;
}
...
void Board::displayBoard()
{
    ...
    std::cout << "Player 1: " <<
                 this->BoardPlayers.at(0).getPhase1Pieces() << " / " <<
                 this->BoardPlayers.at(0).getPhase2Pieces() << " / " <<
                 this-> BoardPlayers.at(0).getFreePieces() << std::endl;
    std::cout << "Player 2: " <<
                 this->BoardPlayers.at(1).getPhase1Pieces() << " / " <<
                 this->BoardPlayers.at(1).getPhase2Pieces() <<" / " <<
                 this-> BoardPlayers.at(1).getFreePieces() << std::endl;
...
vector<std::string> Board::getEmptyLocations()
{
    vector<string> validpositions;
    for (char i = 'a'; i < 'h'; i++)
    {
        for (int j = 0; j < 7; j++)
        {
            if(positions[i-asciiOffset][j].isItValid() &&positions[i-asciiOffset][j].getStatus()==0)
            {
                string temp = i + std::to_string(j+1);
...
vector<std::string> Board::getCurrentPlayerLocations()
{
    vector<string> validpositions;
     for (char i = 'a'; i < 'g'; i++)
     {
         for (int j = 0; j < 7; j++)
         {
             if(positions[i-asciiOffset][j].isItValid() &&positions[i-asciiOffset][j].getStatus()==getCurrentPlayer().getPlayerID())
             {
                 string temp = i + std::to_string(j+1);
                 validpositions.push_back(temp);
             }
...
vector<std::string> Board::getEnemyLocations()
{
    vector<string> validpositions;
    for (char i = 'a'; i < 'g'; i++)
    {
        for (int j = 0; j < 7; j++)
        {
            if(positions[i-asciiOffset][j].isItValid() &&positions[i-asciiOffset][j].getStatus()==3-getCurrentPlayer().getPlayerID())
            {
                string temp = i + std::to_string(j+1);
                validpositions.push_back(temp);
            }
...
vector<std::string> Board::getAMoveInPhase2(std::string start)
{
    vector<std::string> cat;
    return cat;

}

void Board::surrenderGame()
{
    this->state = gameOver;
    std::cout <<"Player " << getCurrentPlayer().getPlayerID() << " surrendered! "
             <<"Player " <<  BoardPlayers.at(getCurrentPlayer().getEnemy() - 1).getPlayerID()
             << " wins! " << std::endl;
}
...
string Board::AIMove()
{
    if(getCurrentPlayer().getIsAI())
    {
     switch (this->state) {
        case phase1:
        {
         string move = getCurrentPlayer().getRandomMove(this->getEmptyLocations());
         char row = move[0];
         int column = move[1] - '0';
         doMovePhase1(row, column, getCurrentPlayer());
         return move;
        }
            break;
        case phase2:
        {
         string start = getCurrentPlayer().getRandomPieceIOwn(this->getCurrentPlayerLocations());
         string goal = getCurrentPlayer().getRandomMove(this->getEmptyLocations());
         while(!isValidLocation(goal[0],goal[1]-'0',start[0],start[1]-'0',getCurrentPlayer()))
         {
             start = getCurrentPlayer().getRandomPieceIOwn(this->getCurrentPlayerLocations());
             goal = getCurrentPlayer().getRandomMove(this->getEmptyLocations());
         }
         char row = start[0];
         int column = start[1] - '0';
         char rowGoal = goal[0];
         int columnGoal = goal[1] - '0';
         doMovePhase2(row, column, rowGoal, columnGoal,this->getCurrentPlayer());
         string finalMove = start+goal;
         return finalMove;
        }
            break;
        case mill:
        {
         string selection = getCurrentPlayer().getRandomEnemyPiece(this->getEnemyLocations());
         while(!removePiece(selection[0], selection[1]-'0',getCurrentPlayer()))
         {
             selection = getCurrentPlayer().getRandomEnemyPiece(this->getEnemyLocations());
         }
         return selection;
        }
            break;
...
void Board::resetBoard()
{
    ...
    for (char i = 'a'; i < 'h'; i+=3)
    {
        ...
        {
            positions[i - asciiOffset][j].setup(i, j + 1, 0, '0', true);
            positions[i - asciiOffset][j].setIsMill(false);
