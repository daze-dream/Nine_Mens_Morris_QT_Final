...
TEST_CASE("Tools to help Play the Game", "[choicesDialog]")
{


    //emulates the choiceDialog box
    choiceDialogTests cat;
    int x = cat.flipCoin();
    REQUIRE(x <=2);
    REQUIRE(x >=1);
    x = cat.randomInteger();
...
TEST_CASE("AI Makin Moves", "[aimoves]")
{
    Board testBoard;
    Player p1(1, false);
    Player p2(2, true);
    ...
    SECTION("vector checks")
    {
        testBoard.doMovePhase1('a',7,testBoard.getCurrentPlayer());
        testBoard.togglePlayer();
        REQUIRE(testBoard.getEmptyLocations().size()==24-1);
        REQUIRE(testBoard.getEnemyLocations().size()==1);
        REQUIRE(testBoard.getCurrentPlayerLocations().size()==0);
        testBoard.AIMove();
        REQUIRE(testBoard.getCurrentPlayerLocations().size()==1);
    }


}

TEST_CASE("Giving up and resetting", "[giveup]")
{
    Board testBoard;
    Player p1(1, false);
    Player p2(2, true);
    ...
    SECTION("resetting the board")
    {
        ...
        //now empty spots is back to normal
        REQUIRE(testBoard.getEmptyLocations().size()==24);
        REQUIRE(testBoard.getCurrentPlayerLocations().size()==0);
        REQUIRE(testBoard.getCurrentPlayer().getPhase1Pieces() == 9);
        REQUIRE(testBoard.getCurrentPlayer().getPhase2Pieces() == 0);

    }

    SECTION("make sure mill status is gone")
    {
            ...
            testBoard.resetBoard();
            //now it shouldnt
            REQUIRE(testBoard.getPositions()['a'-asciiOffset][7-1].isPartofMill() == false);
